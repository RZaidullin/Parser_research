...
void count_time (long int x, struct packet_ctx tmp1) {
    ...
    for (int i = 0; i < x; ++i){
        simple_time = simple_time + s1[i] + s2[i+1] + s3[i+2] + s4[i+3] + s5[i+4] + s6[i+5] + s7[i+6];
        t1 += s1[i];
        t2 += s2[i];
        t3 += s3[i];
        t4 += s4[i];
        ...
        t6 += s6[i];
        t7 += s7[i];
        if (s1[i] >= s2[i] && s1[i] >= s3[i] && s1[i] >= s4[i] && s1[i] >= s5[i] && s1[i] >= s6[i] && s1[i] >= s7[i]) {
            tmp = s1[i];
        }
        else if (s2[i] >= s3[i] && s2[i] >= s4[i] && s2[i] >= s5[i] && s2[i] >= s5[i] && s2[i] >= s6[i]) {
            tmp = s2[i];
        }
        else if (s3[i] >= s4[i] && s3[i] >= s5[i] && s3[i] >= s6[i] && s3[i] >= s7[i]) {
            tmp = s3[i];
        }
        else if (s4[i] >= s5[i] && s4[i] >= s6[i] && s4[i] >= s7[i]) {
            tmp = s4[i];
        }
        else if (s5[i] >= s6[i] && s5[i] >= s7[i]) {
            tmp = s5[i];
        }
        else if (s6[i] >= s7[i]) {
            tmp = s6[i];
    ...
    conv_single[vect_cnt] =  7 * max(t7/double(x), max(t6/double(x), max(t1/double(x), max(t2/double(x), max(t3/double(x), max(t4/double(x), t5/double(x)))))));
